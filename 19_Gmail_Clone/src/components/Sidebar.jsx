    ...
    <div className="w-[15%]">
      ...
      <div className="text-gray-500">
        {sidebarItems.map((item, idx) => {
          return (
            <div onClick={() => setSelected(idx)} key={item.text} className={`${selected === idx ? "bg-[#C2E7FF] text-black" : "hover:bg-gray-200 hover:text-black"} flex pl-6 py-1 rounded-r-full items-center gap-4 my-2  hover:cursor-pointer`}>
              {item.icon}
              <p>{item.text}</p>
            </div>
          );
